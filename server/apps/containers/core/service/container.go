...
func (s *containerService) CreateContainer(ctx context.Context, opts types.CreateContainerOptions) (*types.Container, error) {
	...
	// Set default env
	for _, e := range env {
		err = s.vars.CreateEnv(ctx, types.EnvVariable{
			ID:          uuid.New(),
			ContainerID: id,
...
func (s *containerService) remapDatabaseEnv(ctx context.Context, c *types.Container, options map[string]*types.SetDatabasesOptions) error {
	for databaseID, databaseContainerID := range c.Databases {
		...
		var port, username, password, name string
		for _, v := range dbVars {
			if v.Name == dbEnvNames.Port {
				port = v.Value
			} else if dbEnvNames.Username != nil && v.Name == *dbEnvNames.Username {
				username = v.Value
			} else if dbEnvNames.Password != nil && v.Name == *dbEnvNames.Password {
				password = v.Value
			} else if dbEnvNames.DefaultDatabase != nil && v.Name == *dbEnvNames.DefaultDatabase {
				name = v.Value
			}
		}

		err = s.vars.UpdateEnvByName(ctx, types.EnvVariable{
			ContainerID: c.ID,
			Name:        cEnvNames.Host,
			Value:       host,
		})
